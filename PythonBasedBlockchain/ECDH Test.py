...
Bob_derived_key = HKDF(algorithm=hashes.SHA3_256(), length=size, salt=None, info=b'',).derive(Bob_shared_key)
Alice_derived_key = HKDF(algorithm=hashes.SHA3_256(), length=size, salt=None, info=b'',).derive(Alice_shared_key)

print ("Name of curve: ",Bob_private_key.public_key().curve.name)
print (f"Generated key size: {size} bytes ({size*8} bits)")

bobprivkey = Bob_private_key.private_bytes(Serialize.Encoding.PEM, Serialize.PrivateFormat.TraditionalOpenSSL, Serialize.NoEncryption()).hex()
print (f"\nBob private key value: {bobprivkey}")
bobpubkey = Bob_private_key.public_key().public_bytes(Serialize.Encoding.OpenSSH, Serialize.PublicFormat.OpenSSH).hex()
print("Bob's public key: ",bobpubkey)

vals = Alice_private_key.private_bytes(Serialize.Encoding.PEM, Serialize.PrivateFormat.TraditionalOpenSSL, Serialize.NoEncryption()).hex()
print (f"\nAlice private key value: {vals}")
vals=Alice_private_key.public_key().public_bytes(Serialize.Encoding.OpenSSH, Serialize.PublicFormat.OpenSSH).hex()
